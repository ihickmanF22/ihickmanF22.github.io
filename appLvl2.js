// Level 2
// The comments are the same as appLvl1.js (Level 1) but in relation to Level 2 (this level) and its changes
...
function create() {
  ...
  sprite2.body.setVelocity(100, 200).setBounce(1, 1).setCollideWorldBounds(true);
	...
	sprite3 = this.add.image(300, 50, 'bug3').setScale(0.5);
  ...
  image = this.add.image(600, 300, 'bug3');
  ...
  const container = this.add.container(0, 0, [ wings ]);
	...
	gameState.scoreText2 = this.add.text(154, 13, 'Buggers Left: 24', { fontSize: '15px', fill: '#ebebeb' });
	...
	for (let yVal = 1; yVal < 4; yVal++) {
		for (let xVal = 1; xVal < 9; xVal++) {
			gameState.enemies.create(50 * xVal, 50 * yVal, 'bug2').setScale(.6).setGravityY(-200);
		}
	}
